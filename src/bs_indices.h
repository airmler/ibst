...
class BsInd {
public:
  std::vector<std::vector<int64_t>> tasks;
  std::vector<std::vector<int64_t>> nzcC, nzcA, nzcB;
  std::string cs;
  size_t nTasks;
  ...
  std::string nzcToString(std::vector<int64_t> nzc) {
    std::string out;
  ...
  std::string toStringContract() {
    ...
    auto sizeC(std::set<std::vector<int64_t>>(nzcC.begin(), nzcC.end()).size());
    auto sizeB(std::set<std::vector<int64_t>>(nzcB.begin(), nzcB.end()).size());
    auto sizeA(std::set<std::vector<int64_t>>(nzcA.begin(), nzcA.end()).size());
...
// used in slice/relabel operation
inline BsInd getIndices(std::vector<std::vector<int64_t>> _nzcB,
                        std::vector<std::vector<int64_t>> _nzcA,
                        std::string _cs = "",
                        bool enforce = false) {
  BsInd cind(_cs);
...
inline BsInd getIndices(std::vector<std::vector<int64_t>> _nzcB,
  ...
  cind.tasks.resize(_nzcB.size(), std::vector<int64_t>(2));
  // write tensor B as it is
  for (size_t id(0); id < _nzcB.size(); id++) {
    auto p =
        std::find_if(_nzcB.begin(), _nzcB.end(), [id](std::vector<int64_t> i) {
          return i.back() == id;
        });
  ...
  for (size_t id(0); id < _nzcA.size(); id++) {
    auto t(remap[id]);
    auto p =
        std::find_if(_nzcA.begin(), _nzcA.end(), [t](std::vector<int64_t> i) {
          return i.back() == t;
        });
    assert(p != _nzcA.end());
...
// used in contract operation
inline BsInd getIndices(std::vector<std::vector<int64_t>> _nzcC,
                        std::string _cidxC,
                        std::vector<std::vector<int64_t>> _nzcA,
                        std::string _cidxA,
                        std::vector<std::vector<int64_t>> _nzcB,
                        std::string _cidxB,
                        std::string _cs = "") {
  BsInd cind(_cs);
  ...
  std::vector<int64_t> idxA, idxB;
  std::vector<std::pair<int64_t, int64_t>> ca, cb, ab;
